...
// 辅助: 创建写到 buf 的 Log
func newTestLogger(buf *bytes.Buffer, lv Level) *Log {
...
// Benchmark_Logger_DurFields Dur 字段格式化路径
func Benchmark_Logger_DurFields(b *testing.B) {
	l := New("bench", WithLevel(InfoLevel), WithWriter(io.Discard))
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		l.Info().Dur("cost", time.Duration(i%1e6)*time.Microsecond).Msg("dur")
	}
}
