...
// waitForWorkers 轮询等待 worker 数达到 want, 超时 fatal
func waitForWorkers(t *testing.T, p *GoPool, want int, timeout time.Duration) {
	t.Helper()
	deadline := time.Now().Add(timeout)
	for p.WorkerCount() < uint64(want) {
		if time.Now().After(deadline) {
			t.Fatalf("worker 数未达到 %d, 当前 %d", want, p.WorkerCount())
		}
...
func testGoPoolGoParams(t *testing.T, n int) {
	...
	// 仅断言本测试自建的 pool, 全局池可能被其他 Parallel 测试占用
	if count := pool.TaskCount(); count != 0 {
...
func TestGoPool_WorkerCountAndName(t *testing.T) {
	...
	// 轮询等待 worker 启动, deadline 兜底
	waitForWorkers(t, pool, 1, 5*time.Second)
...
// TestGoPool_WithMaxWorkers 验证 WithMaxWorkers 限制最大 worker 数量
func TestGoPool_WithMaxWorkers(t *testing.T) {
	t.Parallel()

	const maxWorkers = 5
	pool := NewGoPool(WithMaxWorkers(maxWorkers), WithIdleTimeout(10*time.Millisecond))
...
// TestGoPool_WithMaxWorkers_AllTasksDone 验证限制 worker 数量后所有任务仍能完成
func TestGoPool_WithMaxWorkers_AllTasksDone(t *testing.T) {
	t.Parallel()

	pool := NewGoPool(WithMaxWorkers(3), WithIdleTimeout(10*time.Millisecond))
...
func TestGoPool_Shutdown_CompletesQueuedTasks(t *testing.T) {
	...
	pool := NewGoPool(WithMaxWorkers(2), WithIdleTimeout(30*time.Second))
