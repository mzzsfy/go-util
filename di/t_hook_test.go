...
func testMultipleInstanceReverseOrder(t *testing.T) {
	...
	// 验证创建顺序是1,2,3
	// 验证销毁顺序按创建逆序:被依赖者最后销毁
	createOrder := []string{"create-1", "create-2", "create-3"}
	destroyOrder := []string{"destroy-3", "destroy-2", "destroy-1"}
...
func testContainerShutdownHookOrder(t *testing.T) {
	...
	// 主关闭列表按注册逆序执行:
	// 1. before-shutdown（前置钩子最先）
	// 2. 主列表注册顺序为 shutdown-1, shutdown-2, service-destroy
	//    逆序执行: service-destroy, shutdown-2, shutdown-1
...
func testBeforeShutdownPriority(t *testing.T) {
	...
	// 验证：before-shutdown应该在最前面
	// 主关闭列表按注册逆序执行:注册顺序为 shutdown-A, shutdown-B, service-destroy
	// 实际顺序应该是：before-shutdown, service-destroy, shutdown-B, shutdown-A
	...
	if shutdownAIndex != -1 && shutdownBIndex != -1 && shutdownAIndex <= shutdownBIndex {
		t.Errorf("shutdown-B应该在shutdown-A之前执行，但A在%d，B在%d", shutdownAIndex, shutdownBIndex)
	}
}
...
func testShutdownHookReverseOrder(t *testing.T) {
	...
	// 关闭钩子按注册逆序执行:shutdown-5, shutdown-4, shutdown-3, shutdown-2, shutdown-1
	expected := []string{"shutdown-5", "shutdown-4", "shutdown-3", "shutdown-2", "shutdown-1"}
	if len(executionOrder) != len(expected) {
...
func testCompleteShutdownFlow(t *testing.T) {
	...
	// 注册顺序: destroy-1/2/3 在 Get 时注册,shutdown-1/before/shutdown-2 在其后追加
	// 前置钩子最先执行,主列表按注册逆序执行:
	// 1. BeforeShutdown最先执行：before-shutdown ✅
	// 2. 主列表逆序: shutdown-2, shutdown-1, destroy-3, destroy-2, destroy-1
	expected := []string{
		"before-shutdown",          // BeforeShutdown最先
		"shutdown-2", "shutdown-1", // Shutdown钩子按注册逆序
		"destroy-3", "destroy-2", "destroy-1", // 实例按创建逆序销毁,被依赖者最后销毁
	}
