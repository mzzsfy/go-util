...
func (w *TimerWheel) executeTask(t *task) {
	...
	// 原子重置: 与池化上一使用者的 finish CAS 同址, 避免无同步配对
	atomic.StoreUint32(&s.doneCalled, 0)
...
// maxCellTasksCap 槽位切片归还复用的容量上限, 超过后放弃归还交由 GC 回收, 防止尖峰后每槽位长期持有大块内存
const maxCellTasksCap = 1024
...
func (w *TimerWheel) processLayerTasks(layer *wheelLayer, currentIdx int, executeDirectly bool) {
	// 取出当前槽位的任务, 置空槽位, 处理完毕后归还切片容量避免下轮扩容
	...
	// 归还容量: 处理期间若有新任务入槽(已建新切片)或容量超上限, 放弃归还交由 GC 回收
	cell.mu.Lock()
	if cell.tasks == nil && cap(tasks) <= maxCellTasksCap {
		for i := range tasks {
			tasks[i] = nil
		}
		cell.tasks = tasks[:0]
