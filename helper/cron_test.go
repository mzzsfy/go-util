...
// ==================== 内置表达式/时区/@every/@random/日期边界补充 ====================

// Test_ParseCron_FixedExpressions 内置固定表达式的语义
// start 选在 6 月中旬, 避开主流时区的夏令时切换窗口
func Test_ParseCron_FixedExpressions(t *testing.T) {
	t.Parallel()
	start := time.Date(2024, 6, 15, 10, 30, 20, 0, time.Local)
	...
	cases := []struct {
		...
		{"每年", "@annually", []time.Time{time.Date(2025, 1, 1, 0, 0, 0, 0, time.Local)}},
		{"每月", "@monthly", []time.Time{time.Date(2024, 7, 1, 0, 0, 0, 0, time.Local)}},
		{"每周", "@weekly", []time.Time{sunday}},
		{"每日", "@daily", []time.Time{sunday}},
		{"午夜", "@midnight", []time.Time{sunday}},
		{"每小时", "@hourly", []time.Time{time.Date(2024, 6, 15, 11, 0, 0, 0, time.Local)}},
	}
	for _, tc := range cases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			assertNextTime(t, tc.cron, start, tc.nexts...)
		})
	}
}

// Test_ParseCron_TZPrefix 时区前缀解析与时区生效
func Test_ParseCron_TZPrefix(t *testing.T) {
	t.Parallel()
	cases := []struct {
		name string
		cron string
	}{
		{"TZ前缀", "TZ=UTC 0 0 0 1 1 *"},
		{"CRON_TZ前缀", "CRON_TZ=UTC 0 0 0 1 1 *"},
	}
	for _, tc := range cases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			c, err := ParseCron(tc.cron)
			if err != nil {
				t.Fatalf("ParseCron(%q) 不应报错: %v", tc.cron, err)
			}
			next := c.NextTime(time.Date(2024, 6, 15, 10, 0, 0, 0, time.UTC))
			if next.Location() != time.UTC {
				t.Errorf("结果应保留 UTC 时区, 实际 %v", next.Location())
			}
			if !next.Equal(time.Date(2025, 1, 1, 0, 0, 0, 0, time.UTC)) {
				t.Errorf("结果错误: %v", next)
			}
		})
	}
	t.Run("非法时区报错", func(t *testing.T) {
		t.Parallel()
		assertParseError(t, "TZ=Not/AZone 0 0 0 * * *")
	})
	t.Run("前缀后无空格报错", func(t *testing.T) {
		t.Parallel()
		assertParseError(t, "TZ=UTC")
		assertParseError(t, "CRON_TZ=UTC")
		assertParseError(t, "TZ=")
	})
}

// Test_ParseCron_FiveFields 5位表达式自动补秒
func Test_ParseCron_FiveFields(t *testing.T) {
	t.Parallel()
	c, err := ParseCron("30 4 * * *")
	if err != nil {
		t.Fatalf("5位表达式不应报错: %v", err)
	}
	v := c.(*cronTimer)
	if v.second != 1 {
		t.Errorf("秒位应自动补0, 实际 %b", v.second)
	}
	if v.minute != 1<<30 {
		t.Errorf("分钟位错误: %b", v.minute)
	}
	if v.hour != 1<<4 {
		t.Errorf("小时位错误: %b", v.hour)
	}
	if v.day != uint32(math.MaxUint32>>(32-31))<<1 {
		t.Errorf("日位应为全通配: %b", v.day)
	}
	if v.month != (math.MaxUint16>>(16-12))<<1 {
		t.Errorf("月位应为全通配: %b", v.month)
	}
	if v.week != math.MaxUint8>>1 {
		t.Errorf("周位应为全通配: %b", v.week)
	}
}

// Test_ParseCron_EveryInvalid @every 非法参数
func Test_ParseCron_EveryInvalid(t *testing.T) {
	t.Parallel()
	cases := []struct {
		name string
		cron string
...
// Test_ParseCron_RandomArgs @random 参数组合的边界与范围
func Test_ParseCron_RandomArgs(t *testing.T) {
	t.Parallel()
	start := time.Date(2024, 6, 15, 0, 0, 0, 0, time.Local)
	t.Run("双参范围", func(t *testing.T) {
		t.Parallel()
		c, err := ParseCron("@random 2s 5s")
		if err != nil {
			t.Fatalf("不应报错: %v", err)
		}
		// 多次采样覆盖随机区间
		for i := 0; i < 100; i++ {
			d := c.NextTime(start).Sub(start)
			if d < 2*time.Second || d >= 5*time.Second {
				t.Fatalf("随机间隔 %v 超出 [2s, 5s) 范围", d)
			}
		}
	})
	t.Run("参数反序自动交换", func(t *testing.T) {
		t.Parallel()
		c, err := ParseCron("@random 5s 2s")
		if err != nil {
			t.Fatalf("不应报错: %v", err)
		}
		for i := 0; i < 100; i++ {
			d := c.NextTime(start).Sub(start)
			if d < 2*time.Second || d >= 5*time.Second {
				t.Fatalf("反序参数间隔 %v 超出 [2s, 5s) 范围", d)
			}
		}
	})
	t.Run("单参下限为1秒", func(t *testing.T) {
		t.Parallel()
		c, err := ParseCron("@random 2s")
		if err != nil {
			t.Fatalf("不应报错: %v", err)
		}
		for i := 0; i < 100; i++ {
			...
			if d < time.Second || d >= 2*time.Second {
				t.Fatalf("单参间隔 %v 超出 [1s, 2s) 范围", d)
			}
		}
	})
	t.Run("双参相等退化为固定间隔", func(t *testing.T) {
		t.Parallel()
		c, err := ParseCron("@random 2s 2s")
		if err != nil {
			t.Fatalf("不应报错: %v", err)
		}
		if got := c.NextTime(start).Sub(start); got != 2*time.Second {
			t.Errorf("相等参数应退化为固定间隔 2s, 实际 %v", got)
		}
	})
	t.Run("最小值小于1秒报错", func(t *testing.T) {
		t.Parallel()
		assertParseError(t, "@random 500ms 2s")
	})
	t.Run("交换后最小值小于1秒报错", func(t *testing.T) {
		t.Parallel()
		assertParseError(t, "@random 1s 500ms")
	})
	t.Run("最大值非法报错", func(t *testing.T) {
		t.Parallel()
		assertParseError(t, "@random 1s abc")
	})
}

// Test_Cron_NextTime_LeapDay 闰年2月29命中
// 非闰年连续缺 29 日时需持续年份进位, 直到命中下一个闰年的 2月29
func Test_Cron_NextTime_LeapDay(t *testing.T) {
	t.Parallel()
	assertNextTime(t, "0 0 0 29 2 *", time.Date(2023, 6, 1, 0, 0, 0, 0, time.Local),
		time.Date(2024, 2, 29, 0, 0, 0, 0, time.Local),
		time.Date(2028, 2, 29, 0, 0, 0, 0, time.Local))
	// 对照: 2月28每年都存在, 序列连续
	assertNextTime(t, "0 0 0 28 2 *", time.Date(2023, 6, 1, 0, 0, 0, 0, time.Local),
		time.Date(2024, 2, 28, 0, 0, 0, 0, time.Local),
		time.Date(2025, 2, 28, 0, 0, 0, 0, time.Local))
}

// Test_Cron_NextTime_YearRollover 年末秒级进位跨年
func Test_Cron_NextTime_YearRollover(t *testing.T) {
	t.Parallel()
	assertNextTime(t, "59 59 23 31 12 *", time.Date(2023, 12, 31, 23, 59, 59, 0, time.Local),
		time.Date(2024, 12, 31, 23, 59, 59, 0, time.Local))
	assertNextTime(t, "0 0 0 1 1 *", time.Date(2024, 12, 31, 22, 0, 0, 0, time.Local),
		time.Date(2025, 1, 1, 0, 0, 0, 0, time.Local))
}

// Test_Cron_NextTime_ShortMonth 小月缺失 31 号时跳到下一个存在月份
func Test_Cron_NextTime_ShortMonth(t *testing.T) {
	t.Parallel()
	assertNextTime(t, "0 0 0 31 * *", time.Date(2024, 1, 31, 0, 0, 0, 0, time.Local),
		time.Date(2024, 3, 31, 0, 0, 0, 0, time.Local),
		time.Date(2024, 5, 31, 0, 0, 0, 0, time.Local))
}

// Test_Cron_NextTime_WeekdayOnly 仅周几生效时跳到下一个匹配日
func Test_Cron_NextTime_WeekdayOnly(t *testing.T) {
	t.Parallel()
	// 2024-06-15 为周六, 表达式限定周一
	assertNextTime(t, "0 0 0 * * 1", time.Date(2024, 6, 15, 0, 0, 0, 0, time.Local),
		time.Date(2024, 6, 17, 0, 0, 0, 0, time.Local))
}

// Test_ParseCron_WeekSpecialSyntaxReject 周字段特殊语法拒绝
func Test_ParseCron_WeekSpecialSyntaxReject(t *testing.T) {
	t.Parallel()
	cases := []struct {
		name string
		cron string
