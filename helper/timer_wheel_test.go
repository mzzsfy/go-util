...
// firstRecovered 返回首次 panic 记录的 recovered 值, 无记录返回 nil
func (et *execTracker) firstRecovered() any {
	et.mu.Lock()
	defer et.mu.Unlock()
	if len(et.panics) == 0 {
		return nil
	}
	return et.panics[0].Recovered
}
...
func TestTimerWheel_Schedule(t *testing.T) {
	...
	t.Run("ExecutesAfterDelay", func(t *testing.T) {
		...
		// 使用条件等待任务执行完成
		waitForCondition(t, delay+tick*3, func() bool {
			et.mu.Lock()
			count := len(et.tasks)
			et.mu.Unlock()
			return count > 0
	...
	t.Run("BoundaryTiming", func(t *testing.T) {
		...
		for _, tt := range tests {
			...
			t.Run(tt.name, func(t *testing.T) {
				...
				// 使用条件等待任务执行完成
				waitForCondition(t, tt.delay*2+tt.extraWait, func() bool {
					et.mu.Lock()
					hasTimes := len(et.times) > 0
					et.mu.Unlock()
					return hasTimes
				}, "边界延迟任务未执行")
	...
	t.Run("TimeRollback", func(t *testing.T) {
		...
		waitForCondition(t, 300*time.Millisecond, func() bool {
			return atomic.LoadInt32(&et.count) > 0
		}, "任务未执行")
	})
}
...
func TestTimerWheel_ScheduleRepeating(t *testing.T) {
	...
	t.Run("Cancel", func(t *testing.T) {
		...
		// 等待至少执行1次后取消
		waitForCondition(t, interval*2+tick, func() bool {
			return atomic.LoadInt32(&et.count) >= 1
		}, "等待重复任务执行超时")
		handle.Cancel()
	...
	t.Run("PanicSources", func(t *testing.T) {
		...
		for _, tt := range tests {
			...
			t.Run(tt.name, func(t *testing.T) {
				...
				if got := et.firstRecovered(); got != "boom" {
					t.Fatal("panic 处理器未被调用或 panic 值错误", got)
				}
