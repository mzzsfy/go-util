...
func Test_Int64Adder_Overflow(t *testing.T) {
	...
	if adder.Sum() != math.MaxInt64-10 {
		t.Fatalf("初始值应为 %d, 实际 %d", int64(math.MaxInt64-10), adder.Sum())
	}
...
func Test_Int64Adder_Underflow(t *testing.T) {
	...
	if adder.Sum() != math.MinInt64+10 {
		t.Fatalf("初始值应为 %d, 实际 %d", int64(math.MinInt64+10), adder.Sum())
	}
...
func Test_Int64Adder_MaxBoundary(t *testing.T) {
	...
	if adder.Sum() != math.MaxInt64 {
		t.Fatalf("值应为 %d, 实际 %d", int64(math.MaxInt64), adder.Sum())
	}

	// 从最大值减去1
	adder.AddSimple(-1)
	if adder.Sum() != math.MaxInt64-1 {
		t.Errorf("减1后应为 %d, 实际 %d", int64(math.MaxInt64-1), adder.Sum())
	}

	// 再加回去
	adder.AddSimple(1)
	if adder.Sum() != math.MaxInt64 {
		t.Errorf("加1后应为 %d, 实际 %d", int64(math.MaxInt64), adder.Sum())
	}
}
...
func Test_Int64Adder_MinBoundary(t *testing.T) {
	...
	if adder.Sum() != math.MinInt64 {
		t.Fatalf("值应为 %d, 实际 %d", int64(math.MinInt64), adder.Sum())
	}

	// 从最小值加1
	adder.AddSimple(1)
	if adder.Sum() != math.MinInt64+1 {
		t.Errorf("加1后应为 %d, 实际 %d", int64(math.MinInt64+1), adder.Sum())
	}

	// 再减回去
	adder.AddSimple(-1)
	if adder.Sum() != math.MinInt64 {
		t.Errorf("减1后应为 %d, 实际 %d", int64(math.MinInt64), adder.Sum())
	}
}
...
// Reset 与 Add/Sum 并发时的数据竞争: Reset 必须使用 atomic.StoreInt64 写 base
func Test_Int64Adder_ResetRace(t *testing.T) {
	t.Parallel()
	const goroutines = 8
	const iters = 200
...
// 386 平台 base 及 64 位原子字段必须在结构首部保证 8 字节对齐
func Test_Int64Adder_BaseAlignment(t *testing.T) {
	var l Int64Adder
	if off := unsafe.Offsetof(l.base); off%8 != 0 {
		t.Fatalf("base offset=%d 未8字节对齐, 386平台64位原子操作将panic", off)
	}
	if off := unsafe.Offsetof(l.values); off%4 != 0 {
		t.Fatalf("values offset=%d 未对齐", off)
	}
}
