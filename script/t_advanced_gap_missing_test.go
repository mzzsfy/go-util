...
// Test_gap_Context_BindFunc_String 测试BindFunc传入字符串
func Test_gap_Context_BindFunc_String(t *testing.T) {
    defer func() {
        if r := recover(); r == nil {
            t.Error("传入字符串应该panic")
        }
    }()
    ctx := NewContext()
    ctx.BindFunc("test", "not a function")
}

// Test_gap_Context_BindFunc_Int 测试BindFunc传入整数
func Test_gap_Context_BindFunc_Int(t *testing.T) {
    ...
    ctx := NewContext()
    ctx.BindFunc("test", 42)
}

// Test_gap_Context_BindFunc_Nil 测试BindFunc传入nil
