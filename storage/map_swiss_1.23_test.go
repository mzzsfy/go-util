...
// Test_SwissMap_IterDelete_Rehash 验证回调内 Put 触发 rehash 后 del 标记不丢失
// 缺陷: 迭代使用快照 ctrl/groups, 回调内 Put 触发 rehash 后 deleteAt 仍操作旧数组,
// 已标记删除的项在重组后的新表中依然存在, 且 deleteAt 会清空新表对应槽位导致数据损坏
func Test_SwissMap_IterDelete_Rehash(t *testing.T) {
	m := makeSwissMap[int, int](1)
	for i := 0; int(m.Count()) < int(m.limit); i++ {
		m.Put(i, i)
	}
	var seen, inserted []int
	m.IterDelete(func(k, v int) (del, stop bool) {
		seen = append(seen, k)
		nk := 1_000_000 + k
		inserted = append(inserted, nk)
		// 满载表中插入新 key, 稳定触发 rehash
		m.Put(nk, k)
		return true, false
	})
	if len(seen) == 0 {
	...
	for _, k := range seen {
		if _, ok := m.Get(k); ok {
			t.Fatalf("key %d 标记删除后仍存在(rehash 后删除丢失)", k)
		}
	}
	for _, nk := range inserted {
		if v, ok := m.Get(nk); !ok || v != nk-1_000_000 {
			t.Fatalf("rehash 后新 key %d 数据损坏: v=%d ok=%v", nk, v, ok)
		}
	}
}
