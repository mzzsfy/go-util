...
func (m *swissMap[K, V]) IterDelete(cb func(k K, v V) (del bool, stop bool)) bool {
	// 快照当前表结构: 回调内 Put 可能触发 rehash 使 m.ctrl/m.groups 更换底层数组,
	// 此时快照上的位置删除会丢失, 且会破坏新表数据
	ctrl, groups := m.ctrl, m.groups
	g := randIntN(len(groups))
	var deferred []K
	stopped := false
	for n := 0; n < len(groups) && !stopped; n++ {
		for s := 0; s < len(ctrl[g]); s++ {
			...
			if del {
				if len(m.groups) == len(groups) && &m.groups[0] == &groups[0] {
					// 表未被 rehash, 快照即当前表, 直接原地删除(零分配)
					m.deleteAt(g, s, ctrl)
				} else {
					// 已 rehash: 记录 key, 迭代结束后按 key 删除
					deferred = append(deferred, groups[g].keys[s])
				}
			}
			if stop {
				stopped = true
