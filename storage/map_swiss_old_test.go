...
var _ Map[int, int] = &swissMap1[int, int]{}
...
func (m *swissMap1[K, V]) GetSimple(key K) V {
    value, _ := m.Get(key)
    return value
}
...
func (m *swissMap1[K, V]) Delete(key K) {
...
func BenchmarkSwissMapOld(b *testing.B) {
    n := 100
    t1 := []struct {
        name string
        m    func() Map[int, int]
    }{
        {"old___", func() Map[int, int] {
            groups := numGroups(0)
            m := &swissMap1[int, int]{
                ctrl:   make([]metadata, groups),
                groups: make([]group[int, int], groups),
                hash:   NewDefaultHasher[int](),
                limit:  groups * maxAvgGroupLoad,
            }
            for i := range m.ctrl {
                m.ctrl[i] = newEmptyMetadata()
        ...
        {"new", MapTypeSwiss[int, int](0).createMap},
    }
    for _, n1 := range []int{1 << 4, 1 << 12, 1 << 18} {
        for _, m := range t1 {
