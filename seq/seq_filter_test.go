...
// TestBiDistinctByKey 测试BiSeq按key函数去重,与Seq.DistinctByKey对齐
func TestBiDistinctByKey(t *testing.T) {
	t.Parallel()
	seq := BiFromSeq(FromSlice([]int{1, 2, 2, 3, 3, 3, 4}), func(v int) (int, int) {
		return v, v * 10
	})
	var ks, vs []int
	BiDistinctByKey(seq, func(k, v int) int { return k })(func(k, v int) {
		ks = append(ks, k)
		vs = append(vs, v)
	})
	expectedK := []int{1, 2, 3, 4}
	if len(ks) != len(expectedK) {
		t.Fatalf("期望去重后%d个元素, 实际 %d", len(expectedK), len(ks))
	}
	for i, v := range ks {
		if v != expectedK[i] {
			t.Fatalf("索引 %d 期望 K=%d, 实际 K=%d", i, expectedK[i], v)
		}
		if vs[i] != expectedK[i]*10 {
			t.Fatalf("索引 %d 期望 V=%d, 实际 V=%d", i, expectedK[i]*10, vs[i])
		}
	}
}

// TestBiDistinctByKey_CustomKey 测试自定义key函数去重,保留首次出现元素
func TestBiDistinctByKey_CustomKey(t *testing.T) {
	t.Parallel()
	type entry struct {
		ID   int
		Name string
	}
	seq := BiFromSeq(FromSlice([]entry{
		{ID: 1, Name: "a"},
		{ID: 2, Name: "b"},
		{ID: 1, Name: "c"},
		{ID: 3, Name: "d"},
	}), func(v entry) (string, entry) {
		return v.Name, v
	})
	var names []string
	BiDistinctByKey(seq, func(k string, v entry) int { return v.ID })(func(k string, v entry) {
		names = append(names, k)
	})
	expected := []string{"a", "b", "d"}
	if len(names) != len(expected) {
		t.Fatalf("期望去重后%d个元素, 实际 %d", len(expected), len(names))
	}
	for i, v := range names {
		if v != expected[i] {
			t.Fatalf("索引 %d 期望 %s, 实际 %s", i, expected[i], v)
		}
	}
}

// TestBiDistinctByKey_Empty 测试空序列
func TestBiDistinctByKey_Empty(t *testing.T) {
	t.Parallel()
	seq := BiFromSeq(FromSlice([]int{}), func(v int) (int, int) { return v, v })
	count := 0
	BiDistinctByKey(seq, func(k, v int) int { return k })(func(k, v int) {
		count++
	})
	if count != 0 {
		t.Fatalf("空序列结果应为空, 实际 %d", count)
	}
}
